...
void extract_forest(const std::string forestdir, std::string& metadata,
                    std::vector<std::string>& trees) {
  struct dirent* entry = nullptr;
  DIR* dp              = nullptr;

  dp = opendir(forestdir.c_str());
  if (dp != nullptr) {
    while ((entry = readdir(dp))) {
      std::string filename = entry->d_name;
      if (filename.find("metadata") != std::string::npos) {
        metadata = forestdir + "/" + filename;
      } else if (filename.find("tree") != std::string::npos) {
        trees.push_back(forestdir + "/" + filename);
      }
    }
  } else {
    std::cout << "Directory (" << forestdir << ") failed to open!" << std::endl;
    exit(1);
  }
  closedir(dp);
}
...
int main(int argc, char* argv[]) {
  Morpheus::initialize(argc, argv);
  {
    if (argc != 5) {
      ...
      rt_error_msg
          << "\tdforest_base  : Baseline Random Forest file to be used.\n";
      rt_error_msg
          << "\tdforest_tuned : Tuned Random Forest file to be used.\n";
      rt_error_msg
    ...
    std::string fmatrix = argv[1], dforest_base = argv[2],
                dforest_tuned = argv[3], outdir = argv[4];

    std::cout << "\nRunning decision_tree_tuner example with:\n";
    std::cout << "\tMatrix Filename  : " << fmatrix << "\n";
    std::cout << "\tBaseline Forest Directory    : " << dforest_base << "\n";
    std::cout << "\tTuned Forest Directory    : " << dforest_tuned << "\n";
    ...
    // Baseline
    {
      std::string metadata;
      std::vector<std::string> trees;
      extract_forest(dforest_base, metadata, trees);

      auto start = std::chrono::steady_clock::now();
      Morpheus::Oracle::RandomForestTuner tuner(metadata, trees, true);
